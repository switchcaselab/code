...
_start:
    ...
    mov rdi, 2
    lea rsi, _InteruptSignal
    call RegisterSignalHandler
    ...
    3:
        ...
        mov rdi, 0 # Exit with status code
        call Exit
    ...
    _ChildProgram:        
        # Allocate 4 bytes for the client socket [rsp]
        # Allocate 4 bytes for the file descriptor [rsp + 4]
        # Allocate 1024 bytes for the read buffer [rsp + 8]
        # Allocate 1024 bytes for the file path [rsp + 1032]
        sub rsp, 2056
        mov [rsp], edi # save socket fd
        4: 
            # Read the client request
            mov edi, DWORD PTR [rsp] # fd = client socket
            lea rsi, [rsp + 8]
            mov rdx, 1024        
            call Read
            cmp eax, 1024
        ...
        4:
            mov edi, DWORD PTR [rsp + 4] # fd = file fd
            lea rsi, [rsp + 8]
            mov rdx, 1024
            call Read
...
_InteruptSignal:
    mov edi, [server_socket] # prepare to close the socket stored in [rsp].
    call Close # call the close syscall
    mov rdi, 0
    call Exit
